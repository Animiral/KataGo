...
public:
  ...
  void backward(Output output, Output target, float learnrate);  // buffers must be filled by forward pass

private:

  static const uint32_t STRNET_HEADER;
  static constexpr std::size_t N = 1000; // max number of input moves (for workspace buffer)
  ...
  float* outputx;  // memory holding output features before aggregation
